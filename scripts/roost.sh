...
generate_cert() {
  ...
  #if Roost versio is latest then change GIT_URL
...
main() {
  ...
  if [ ! -z "${SCRIPT}" ]; then
    if [ "${SCRIPT}" == "jumpHost.sh" ]; then
      setup_jump
    elif [ "${SCRIPT}" == "singleHost.sh" ]; then
...
setup_singleInstance() {
  ...
  for script in ec2Launcher.sh aiServer.sh releaseServer.sh
  do
