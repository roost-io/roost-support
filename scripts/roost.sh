...
expose_docker_on_port() {
	...
	if [ ! -d "/var/tmp/Roost/docker" ]; then
        mkdir -p /var/tmp/Roost/docker
		chmod 775 /var/tmp/Roost/docker
    fi
    ...
    cat <<EOF > /etc/docker/daemon.json
{
  ...
  "data-root": "/var/tmp/Roost/docker"
}
EOF
...
install_docker() {
    ...
    if systemctl is-active --quiet docker; then
        echo "INFO: Docker is already running. Skipping daemon.json modifications."
        # Only add roost user to docker group
...
setup_aiserver() {
    ...
    check_aiServer
    pkill -f "${SCRIPT}" || true
    local archive="aiServer gotty"
    copy_archive $archive
    local scripts="aiServer.sh master.tar.gz roost.sh aiServer.commitid userOrganisationCluster.sh"
    copy_scripts $scripts
}
