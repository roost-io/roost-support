...
ensure_permissions() {
    ...
    [ -d "${ROOST_BIN}" ] && find "${ROOST_BIN}" -type f -exec chmod 755 {} \;
}
# Mount the EBS disk
mount_ebs() {
    local epoch=$(date +%s)
...
create_folders() {
    ...
    # Apply ownership to entire tree
    chown -R ${ROOST_UID}:${ROOST_UID} "${ROOST_DIR}"
    ...
    # Sensitive directories need restricted access
    chmod 700 "${ROOST_DIR}/roostai_nestjs_data"
    chmod 700 "${ROOST_DIR}/.ssh"
    chmod 700 "${ROOST_DIR}/.roost"
    
    # Ensure roost owns sensitive directories
    chown -R ${ROOST_UID}:${ROOST_UID} "${ROOST_DIR}/roostai_nestjs_data"
    chown -R ${ROOST_UID}:${ROOST_UID} "${ROOST_DIR}/.ssh"
    chown -R ${ROOST_UID}:${ROOST_UID} "${ROOST_DIR}/.roost"
}
...
copy_archive() {
    for binary in "$@"; do
        ...
        if [ -s "$HOME/${binary}.gz" ]; then
            cp -f "$HOME/${binary}.gz" "${ROOST_BIN}/${binary}.gz"
            gunzip -f "${ROOST_BIN}/${binary}.gz"
        elif [ -s "$HOME/${binary}" ]; then
            cp -f "$HOME/${binary}" "${ROOST_BIN}/${binary}"
        fi
...
setup_ec2launcher() {
    ...
    local scripts="releaseServer.sh aiServer.sh ec2Launcher.sh master.tar.gz roost.sh releaseServer.commitid ec2launcher.commitid aiServer.commitid"
