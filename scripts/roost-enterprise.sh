...
write_crontab() {
  cat <<EOF > $ROOST_BIN/nestLogs.sh
#!/bin/bash
mydate=\$(date +%Y%m%d-%H:%M)
echo \$mydate >> ${ROOST_LOG}/nestJs.log
echo $ENTERPRISE_DNS >> ${ROOST_LOG}/nestJs.log
/usr/bin/docker logs roost-roostai_nest_server-1 --since 1h | egrep -v '^query:' >> ${ROOST_LOG}/nestJs.log
echo "==== Done ===" >> ${ROOST_LOG}/nestJs.log
cd ${ROOST_LOG}

/usr/bin/curl --location "https://${ENTERPRISE_DNS}/api/test/support" \
--form files=@"${ROOST_LOG}/nestJs.log" \
--form 'tpAppId="${ENTERPRISE_NAME}"' \
--form 'contact_email="support@roost.ai"' \
--form roost_domain="$ENTERPRISE_DNS" \
--form 'additional_support_message="nest js container logs"' \
--form 'use_type="script"'

if [ \$? -eq 0 ]; then
  rm -f ${ROOST_LOG}/nestJs.log
fi
EOF
  chmod +x $ROOST_BIN/nestLogs.sh
  (crontab -l; echo "10 * * * * ${ROOST_BIN}/nestLogs.sh >> ${ROOST_LOG}/nestLogs.out") | awk '!x[$0]++' | crontab -
}
