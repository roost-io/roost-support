...
read_and_check_env_file(){
    ...
    ([[ "$OTP_LOGIN_ENABLED" == true ]] && [[ "$EMAIL_SENDER_PRESENT" == false ]]) && { echo "Add EMAIL_SENDER and EMAIL_SENDER_PASS in env file to enable OTP based login"; isFileRight=false ;}
...
EOF
  ...
  case $ec2_type in
    ...
    "rhel")
      ...
      sudo cp $ENTERPRISE_CERTIFICATE_KEY_PATH /etc/pki/nginx/private/server.key
      sudo cp $ENTERPRISE_CERTIFICATE_PATH /etc/pki/nginx/server.crt
      ENTERPRISE_CERTIFICATE_KEY_PATH="/etc/pki/nginx/private/server.key"
      ENTERPRISE_CERTIFICATE_PATH="/etc/pki/nginx/server.crt"
      ;;
  ...
  # TODO:
  # if RHEL either install nginx-plus-module-headers-more OR remove more_clear_headers
  # Create folder /etc/pki/nginx/private
  #     and copy files to the /etc/pki/nginx/server.cer or as server.crt
  #     and /etc/pki/nginx/private/server.key
...
server {
    ...
    location / {
        ...
        add_header Content-Security-Policy "default-src 'self'; style-src 'self' 'sha256-47DEQpj8HBSa+/TImW+5JCeuQeRkm5NMpJWZG3hSuFU=' 'sha256-DLDPR1ic47WIdK2WyeLkblb/tm2mQH+Jt/NNhZWu1k0=' 'sha256-OLwcmpBt8IjBga/VtyUBqC7LBEXUM0W0W9ANcXobffo=' 'sha256-u/2DZKYp97wyctK3jrAd90/uZpUEE9FWZKd/uCvBwKc='; script-src 'self' 'unsafe-eval'; img-src 'self' data: https://roost.ai; connect-src 'self' https://api.openai.com https://*.okta.com https://*.amazonaws.com https://*.azure.com https://*.roost.ai; " always;
...
apply_docker_compose(){
  ...
  ec2_type=$(cat /etc/os-release | grep '^ID='| cut -f2 -d'='|sed -e 's/"//g')
  rhel_container_exe="docker" # podman
  case $ec2_type in
    "ubuntu")
    ...
    ;;
    "rhel")
      sudo ${rhel_container_exe} compose -f $ROOST_DIR/docker-compose.yaml pull
      sudo ${rhel_container_exe} compose -f $ROOST_DIR/docker-compose.yaml up -d --build --remove-orphans --force-recreate
...
restart_docker_compose(){
  ...
  case $ec2_type in
    ...
    "rhel")
      ...
      echo "${rhel_container_exe} compose up: $?"
      ;;
    *) echo "container restart up is not configured for $ec2_type";;
  esac
  if [ ! -z "$DEV" -a "$DEV" == "1" ]; then
      touch "${ROOST_DIR}/.dev"
  elif [ -f "${ROOST_DIR}/.dev" ]; then
...
docker_prune() {
  ...
  case $ec2_type in
    ...
    "rhel")
      rhel_container_exe="docker" # podman
      sudo ${rhel_container_exe} image prune --all
      ;;
    *)
...
case ${install} in
    ...
    "gpt")
      ...
      ;;
    "ec2")
      echo "install roost web, db, gpt and controlplance server on a single ec2"
      roost_controlplane
      ;;
    "roostai")
