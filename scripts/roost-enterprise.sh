...
read_and_check_env_file(){
    ...
    ROOST_DEVELOPMENT=$(echo "$fileContent" | jq -r '.roost_development')
    RATE_LIMIT_MAX=$(echo "$fileContent" | jq -r '.rate_limit_max')
...
EOF
    ...
    if [ "$ROOST_DEVELOPMENT" == "true" ]; then
        cat >> /tmp/server.env << EOF
ROOST_DEVELOPMENT=true
EOF
    fi
    if [ -n "$RATE_LIMIT_MAX" ] && [ "$RATE_LIMIT_MAX" != "null" ]; then
        cat >> /tmp/server.env << EOF
...
GITHUB_CLIENT_ID=${GITHUB_CLIENT_ID:-}
GITHUB_CLIENT_SECRET=${GITHUB_CLIENT_SECRET:-}
GITHUB_REDIRECT_URI=${GITHUB_REDIRECT_URI:-"https://${ANALYZER_DNS}/api/auth/github/callback"}

JWT_SECRET_KEY=${JWT_SECRET:-"sdfloencgekdyencpqzgdkrtcldyr"}
JWT_ALGORITHM=${ANALYSER_JWT_ALGORITHM:-HS256}
JWT_EXPIRATION_HOURS=${ANALYSER_JWT_EXPIRATION_HOURS:-24}

API_HOST=0.0.0.0
API_PORT=5060
API_RELOAD="false"

DB_HOST=ai_code_analyzer_db
DB_PORT=5432
DB_NAME=code_parser
DB_USER=postgres
DB_PASSWORD=12345678

DATABASE_URL=postgresql://postgres:12345678@ai_code_analyzer_db:5432/code_parser

CHROMA_PERSIST_DIR=/app/chroma_db
JOBS_STORAGE_DIR=/app/jobs
EOF
    run_as_root mv /tmp/analyzer.env "${ROOST_DIR}/.roost/analyzer.env"
    run_as_root chown ${ROOST_UID}:${ROOST_UID} "${ROOST_DIR}/.roost/analyzer.env"
    run_as_root chmod 660 "${ROOST_DIR}/.roost/analyzer.env"
}
...
server {
    listen 80;
    server_name $ENTERPRISE_DNS;
    return 301 https://$ENTERPRISE_DNS\$request_uri;
}

server {
    listen 80;
    server_name $ANALYZER_DNS;
    return 301 https://$ANALYZER_DNS\$request_uri;
}
...
# Code analyzer isolated subdomain
server {
    listen 443 ssl;
    server_name $ANALYZER_DNS;
...
services:
  ...
  ai_code_analyzer:
    image: zbio/ai-io:latest
    container_name: ai-test-intelligence-backend
    user: "10001:10001"
    ports:
      - "5062:5060"
    env_file:
    - $ROOST_DIR/.roost/analyzer.env
    volumes:
      - $ROOST_DIR/code-analyzer/chroma_db:/app/chroma_db
      - $ROOST_DIR/code-analyzer/jobs:/app/jobs
      - $ROOST_DIR/code-analyzer/test_artifacts:/app/test_artifacts
    depends_on:
      - ai_code_analyzer_db
    restart: unless-stopped
    networks: 
  ...
  ai_code_analyzer_db:
    ...
    container_name: ai-test-intelligence-db
    environment:
      POSTGRES_DB: code_parser
    ...
    ports:
      - "5431:5432"
    volumes:
    - $ROOST_DIR/code-analyzer/postgres_data:/var/lib/postgresql/data
    healthcheck:
