...
read_and_check_env_file(){
    ...
    EAAS_SERVER_IP=$(echo $fileContent | jq -r '.eaas_server_ip')
    EAAS_SERVER_KEY_PATH=$(echo $fileContent | jq -r '.eaas_server_pem_key')
    EAAS_SERVER_USERNAME=$(echo $fileContent | jq -r '.eaas_server_username')
    ...
    ([[ -z "$USE_AS_ROOST_CLOUD_SERVER" ]] || [[ "$USE_AS_ROOST_CLOUD_SERVER" = null ]]) && { USE_AS_ROOST_CLOUD_SERVER=false ;}
    ...
    ([[ $githubLogin = false ]] && [[ $azureLogin = false ]] && [[ $linkedinLogin = false ]] && [[ $oktaLogin = false ]] && [[ $googleLogin = false ]] && [[ $azureAdfsLogin = false ]] && [[ $USE_NO_AUTH = false ]] )  && { echo "Add atleast one third party client id"; isFileRight=false ;}
...
get_roost_files() {
  ...
  dest=$2
  if [ -z "${dest}" ]; then
    dest=${ROOST_BIN}/${s3_file}
  fi
  if [ -x "${ROOST_BIN}/$INSTALLER" ]; then
    # TODO: Allow for a download destination as well or for copying/moving the file post download
    ${ROOST_BIN}/$INSTALLER --command=download ${s3_file}
    if [ ! -s "$dest" ]; then
      echo "Failed to download the file $s3_file, exit program"
      exit 1
    fi
  else
    echo "Failed: get_roost_files_compat ${s3_file} ${dest}"
  fi
}

xxget_roost_files_compat() {
  s3_file=$1
  dest=$2
  if [ -z "${dest}" ]; then
    dest=${ROOST_BIN}/${s3_file}
  ...
  if [ "$DEV" != "1" -a ! -f "${ROOST_DIR}/.dev" ]; then
    S3_CONFIG_URL=https://remote-roostprod.s3-us-west-1.amazonaws.com
  fi
  curl -q -s ${S3_CONFIG_URL}/${s3_file} -o ${dest}
  # chown ubuntu ${ROOST_BIN}/${dest}
  chmod +x ${dest}
}
...
install_stun(){
    install_docker
    if [ ${install} == "stun" ]; then
      sudo docker stop roost_stun_svr
      sudo docker rm roost_stun_svr
      sudo docker run -d -p 2502:2502 -e ROOST_IO_SVR=$ENT_DNS --name roost_stun_svr --restart unless-stopped zbio/roost-stun-svr:v2
    fi
}

#check prereqs stun variables
check_prereqs_stun(){
    if [ -z "${RoostIOServer}" ]; then
        echo "Error:RoostAI endpoint value is compulsory to install stun"
        other_options
        exit 1
    fi
}

#check prereqs cluster launcher variables
check_prereqs_cluster_launcher(){
    if [ -z "${APP_NAME}" ]; then
      if [ -z "$AppName"]; then
        echo "Error:AppName is compulsory to install cluster launcher"
        other_options
        exit 1
      fi
      APP_NAME=$AppName
...
roost_controlplane() {
  ...
  if [ ${ECS_MODE} == true ]; then
    get_roost_files master.tar.gz
    tar -xvzf ${ROOST_BIN}/master.tar.gz -C ${ROOST_DIR} yaml;

    # get_roost_files DockerApiCompose.yaml
    # get_roost_files roost-docker-monitor.yml
    # mv ${ROOST_BIN}/DockerApiCompose.yaml ${ROOST_BIN}/roost-docker-monitor.yml ${ROOST_DIR}/yaml

    find ${ROOST_DIR}/yaml -type 'f' -name '*.yaml' -exec sed -i -e "s/TAG/${ROOST_VER}/g" {} \;
    find ${ROOST_DIR}/yaml -type 'f' -name '*.yml' -exec sed -i -e "s/TAG/${ROOST_VER}/g" {} \;

    sudo tar -xvzf ${ROOST_BIN}/master.tar.gz -C /var/tmp bin;
    cp -f /var/tmp/bin/RoostApi.gz /var/tmp/bin/RoostK8s.gz /var/tmp/bin/RoostMetricsDb ${ROOST_BIN}

    # get_roost_files RoostApi.gz
    # get_roost_files RoostMetricsDb
    # get_roost_files RoostK8s.gz

    gunzip -f /var/tmp/Roost/bin/RoostApi.gz
    gunzip -f /var/tmp/Roost/bin/RoostK8s.gz

    get_roost_files roost-docker.sh
    get_roost_files jumpHost.sh
    get_roost_files deleteJumphost.sh
    get_roost_files k3d_install.sh

    curl -s https://roost-stable.s3.us-west-2.amazonaws.com/enterprise/roost.sh | TAG=$ROOST_VER SCRIPT=jumpHost.sh bash -
    sudo ROOST_LOCAL_KEY=$AUTH_KEY RUN_AS_CONTAINER=true ENT_DNS=https://${ENTERPRISE_DNS}/api ONLY_DOCKER=1 ${ROOST_BIN}/jumpHost.sh
  fi
