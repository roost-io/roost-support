...
main() {
  ...
  # Also restart if running aiServer is missing the expected entServer arg
  if [ $grep_status -eq 0 ] && [ -n "${ENTSERVER}" ]; then
    if ! pgrep -a aiServer | grep -q "entServer=${ENTSERVER}"; then
      echo "aiServer running but entServer mismatch, restarting"
      grep_status=1
    fi
  fi
  if [ $grep_status -ne 0 ]; then
